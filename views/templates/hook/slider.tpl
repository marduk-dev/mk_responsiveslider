{if $mk_slider.slides}
  <div id="{$mk_slider.slider_id}" class="carousel slide" data-bs-ride="carousel" data-bs-interval="{$mk_slider.speed}" data-bs-wrap="{(string)$mk_slider.wrap}" data-bs-pause="{$mk_slider.pause}">
    ...
    {foreach from=$mk_slider.slides item=slide key=idxSlide name='mk_slider'}
      ...
      {if !empty($slide.url)}<a href="{$slide.url}">{/if}
        <img src="{$slide.desktop_img}" class="d-block w-100" alt="{$slide.legend|escape}" loading="lazy">
